...
queue<node*> breadthfirstsearch(node* first,int val){
	node* start = first;
	queue<node*> open; // to store nodes to be visited
	queue<node*> closed; //  to store nodes which are already visited
	open.push(start);
	queue<node*> em; // empty queue to be returned if value not found.
	while(!open.empty()){
